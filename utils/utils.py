...
# Simulate an input tensor (batch_size=1, channels=3, height=120, width=200)
image = torch.randn(1, 3, 120, 200)

# Initialize the padder
padder = InputPadder(image.shape)

# Pad the image
padded_image = padder.pad(image)[0]  # Use index 0 to get the first padded tensor

print("Original size:", image.shape)
print("Padded size:", padded_image.shape)

# Unpad the image after processing
unpadded_image = padder.unpad(padded_image)[0]
print("Unpadded size:", unpadded_image.shape)

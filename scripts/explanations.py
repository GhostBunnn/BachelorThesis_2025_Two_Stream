...
def process_video(video_folder, stream, model, transform, data_dir, device, global_saliency_storage=None):
    ...
    out_dir = os.path.join(BASE_DIR, "plots", "saliency_maps", f"{stream}_{video_folder}")
    ...
    #     sal_flow = vanilla_saliency(model, flow_tensor)[0].detach().cpu().numpy()

    #     for i in range(0, sal_flow.shape[0], 2):
    #         sal_pair = np.mean(sal_flow[i:i+2], axis=0)
    #         save_saliency_map(sal_pair, f"Temporal Saliency channels {i}+{i+1}",
    #                           os.path.join(out_dir, f"saliency_{i}+{i+1}.png"))
...
if __name__ == "__main__":
    ...
    spatial_model = load_spatial_model_with_dropout(101, os.path.join(BASE_DIR, "saved_models", "spatial_model_lr0.0001_bs25_epochs25_03.pth"), device)
    temporal_model = load_temporal_model(101, os.path.join(BASE_DIR, "saved_models", "temporal_model_lr0.0005_bs25_epochs25_03.pth"), device)
    ...
    save_average_across_videos(global_saliency, os.path.join(BASE_DIR, "plots", "saliency_maps", "spatial_summary"))

...
TRAIN_SPLIT = os.path.join(BASE_DIR, "data", "splits", "trainlist03_processed.txt")   # can be changed with 01, 02, 03 at the end for different files
...
MODEL_SAVE_PATH = os.path.join(BASE_DIR, "saved_models", f"spatial_model_lr{learning_rate}_bs{batch_size}_epochs{num_epochs}_03.pth")
...
# Dataset preparation
train_dataset = RGBDataset(DATA_DIR, TRAIN_SPLIT, transform=transform)  # Updated to include split_file
val_dataset = RGBDataset(DATA_DIR, VAL_SPLIT, transform=transform)      # Updated to include split_file
test_dataset = RGBDataset(DATA_DIR, TEST_SPLIT, transform=transform)    # Updated to include split_file
...
combined_plot_path = os.path.join(BASE_DIR, "spatial_combined_plot_lr5e4.png")
